...
- Add incrementing number for new commands
...
function MainWindow {
    ...
    $App.UI.BtnClose.Add_Click({ BtnCloseWindowClick $App.UI.Window })
    $App.UI.BtnMinimize.Add_Click({ BtnMinimizeWindowClick $App.UI.Window })
    $App.UI.BtnMaximize.Add_Click({ BtnMaximizeWindowClick $App.UI.Window })
    ...
    $App.UI.Window.Dispatcher.InvokeAsync{ $App.UI.Window.ShowDialog() }.Wait()

    # Cleanup instance when done
    $App.PS.Runspace.Close()
    $App.PS.Runspace.Dispose()
    $App.PS.EndInvoke()
}
...
function BuildCommandGrid($Grid, $Parameters) {
    for ($i = 0; $i -lt $Parameters.Count; $i++) {
        ...
        SetGridPosition -Element $box -Row $i -Column 1
    }
}
